...
ProcessTouch(vector st) {
        integer col = (integer)(st.x * 8.0);
        integer row = (integer)(st.y * 8.0);
        integer sq = (row * 8) + col;
        integer color = !((row + col) & 1); // Even squares are black, odd are white
    ...
    touch_start(integer num_detected) {
        ...
        for (i = 0; i < num_detected; i++) {
            if (llDetectedKey(i) == gValidToucher) {
                ProcessTouch(llDetectedTouchST(i));
