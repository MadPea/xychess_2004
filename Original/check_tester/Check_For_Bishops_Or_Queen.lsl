///////////////////////////////////////////////////////////////////////////////////////////
// Check For Bishops Or Queen Script
//
...
integer CheckForBishopsOrQueen() {
    // Check the diagonals bishops or queen.
    // Check going up and right.
    integer RowCheck = gRow + 1;
    integer ColCheck = gCol + 1;
    integer Done = FALSE;
    integer Piece;
    integer Move;
    while (!Done) {
        Move = GetIndex(RowCheck, ColCheck);
        Piece = GetPieceByIndex(Move);

        // See if this is an enemy bishop or queen.
        if (Piece == gEnemyBishop || Piece == gEnemyQueen)
            return TRUE;

        // Make sure there wasn't another piece to block,
        // and that we are still on the board.
        if (Piece != NONE || Move == NONE)
            Done = TRUE;
...
default {
    state_entry() {
        ...
        gTestCheckChannel = CHECK_FOR_BISHOPS_OR_QUEEN + gTestID;
    }
